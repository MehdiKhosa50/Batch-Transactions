...
contract BatchLimitOrderRouter is Ownable(msg.sender) {
    ...
    function executeOrder(Order memory order) internal returns (bool) {
        ...
        // Check expected output amount and price range
        address[] memory path = new address[](2);
        path[0] = order.tokenIn;
        path[1] = order.tokenOut;

        try router.getAmountsOut(order.amountIn, path) returns (
            uint256[] memory amounts
        ) {
            uint256 currentPrice = (amounts[1] * 1e18) / order.amountIn; // Normalized price calculation

            if (
                currentPrice < order.lowPrice || currentPrice > order.highPrice
            ) {
                emit OrderExecuted(
                    order.maker,
                    order.tokenIn,
        ...
        try
            tokenIn.transferFrom(order.maker, address(this), order.amountIn)
        ...
        // Approve router
        try tokenIn.approve(address(router), order.amountIn) {} catch {
            emit OrderExecuted(
                order.maker,
                order.tokenIn,
                order.tokenOut,
